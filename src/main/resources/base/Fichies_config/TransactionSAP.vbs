...
' 1: Code transaction (ex: /nCS03)
' 2: Valeur (dépend de la transaction)
' 3: Nom d'utilisateur SAP (optionnel)
' 4: Mot de passe SAP (optionnel)
...
'=== 3. Gestion des identifiants SAP en arguments
If WScript.Arguments.Count >= 4 Then
    sapUsername = WScript.Arguments(2)
    sapPassword = WScript.Arguments(3)
Else
    ' Charger les identifiants depuis le registre si non fournis en arguments
    GetSavedCredentials()
End If
...
' Fonction pour terminer le script avec un message d'erreur
Sub TerminateScript(message)
    MsgBox message, vbExclamation, "Erreur"
    WScript.Quit 1
End Sub
