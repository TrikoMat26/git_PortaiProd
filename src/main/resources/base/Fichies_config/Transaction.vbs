...
For i = 0 To ZP20.length - 1
    ...
    valZREFF  = session.findById("wnd[0]/usr/cntlTREE_CONTAINER/shellcont/shell").getItemText(Key, "ZREFF")

    ' Utiliser la valeur ZTOPO comme clé et stocker un tableau contenant ZCOMP, ZDES et ZREFF
    If Not dict.Exists(valZTOPO) Then
        dict.Add valZTOPO, Array(valZCOMP, valZDES, valZREFF)
    End If
Next
...
' Format de sortie : [["ZTOPO", "ZCOMP", "ZDES", "ZREFF"], ...]
